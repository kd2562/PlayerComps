...
function RadarPlot({sortedVals, aggData, playerId, index}) {
	...
	const [svgWidth, setSVGWidth] = useState(document?.body?.offsetWidth/2.2 || 500)
	const [svgHeight, setSVGHeight] = useState(document?.body?.offsetWidth/2.2 || 500)

	// Redraw plot on window resize
	useEffect(() => {
		if(document?.body){
			setSVGWidth(document.body.offsetWidth/2.2)
			setSVGHeight(document.body.offsetWidth/2.2)
		}
